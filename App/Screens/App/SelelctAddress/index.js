...
const SelelctAddress = ({ navigation, route }) => {
    const { totalAmount, couponCodeData, couponDis, DeliveryCharge, taxDeliveryData, tax } = route.params
    ...
    const [checkSaturday, setCheckSaturday] = useState(6);
    ...
    const [timeshowData, setTimeShowData] = useState('')
    ...
    const onSelelctPaymentMethod = async () => {
        ...
        if (valid === true) {
            navigation.navigate('SelelctPaymentMethod', { totalAmount, couponCodeData, couponDis, selectaddressData, DeliveryCharge, taxDeliveryData, tax, time, date })
        }
    }
    const onTimeAndDate = async () => {
        setModalVisible(false)
        navigation.navigate('SelelctPaymentMethod', { totalAmount, couponCodeData, couponDis, selectaddressData, DeliveryCharge, tax, taxDeliveryData, time, date })
    }
    const editAddress = () => {
    ...
    function validationForm() {
        ...
        if (moment(date).weekday() == checkSunday) {
            ...
            setMonSatMsg(true)
            return false;
        } else if (moment(date).weekday() == checkSaturday) {
            setDateVisible(true)
            setMonSatMsg(true)
            return false;
        } else {
        ...
        var currentDate = moment(currentTime).format("MMM Do YYYY")
        var selectDate = moment(date).format("MMM Do YYYY")
        if (currentDate == selectDate) {
            ...
            if (times.isBetween(beforeTime, afterTime)) {
                setPastTime(false)
                console.log('patidar2')
            } else {
                console.log('patidar3')
                setMonSatMsg(true)
                setModalVisible(true)
                setPastTime(false)
                return false;
    ...
    const onChangeDateios = (event, selectedValue) => {
        setDateValue(selectedValue)
    };
    const onChangeDateSaveIos = () => {
        setDate(dateValue);
        setDateShowVisible(!dateShowVisible)
        setTime('');
        setShowDate(false);
    ...
    const onChangeTimeIos = (event, selectedValue) => {
        setTimeShowData(selectedValue)
        setTimeValue(selectedValue);
    };

    const onChangeTimeSaveIos = () => {
        setShowTime(false);
        setTime(timeshowData);
        setTimeShowVisible(!timeShowVisible);
    };
                    ...
                    <View style={styles.timeModelCon}>
                        <View style={styles.timeInnderCon} >
                            ...
                            <View style={{ justifyContent: 'center', alignItems: 'center' }}>
                                <TouchableOpacity onPress={() => onChangeTimeSaveIos()} style={{ height: 30, width: 90, borderWidth: 1, justifyContent: 'center', alignItems: 'center', borderRadius: 8, borderColor: PINK_COLOR_CODE }}>
                                    <Text style={{ fontFamily: FONT_FAMILY_REGULAR, color: PINK_COLOR_CODE, fontSize: 18 }}>Save</Text>
                                </TouchableOpacity>
                            </View>
                        </View>
                    </View>
                ...
                >
                    <View style={styles.timeModelCon}>
                        <View style={styles.timeInnderCon} >
                            ...
                            <View style={{ justifyContent: 'center', alignItems: 'center' }}>
                                <TouchableOpacity onPress={() => onChangeDateSaveIos()} style={{ height: 30, width: 90, borderWidth: 1, justifyContent: 'center', alignItems: 'center', borderRadius: 8, borderColor: PINK_COLOR_CODE }}>
                                    <Text style={{ fontFamily: FONT_FAMILY_REGULAR, color: PINK_COLOR_CODE, fontSize: 18 }}>Save</Text>
                                </TouchableOpacity>
                            </View>
                        </View>
                ...
                <View style={styles.timeModelCon}>
                    <View style={styles.timeInnderCon} >
                        <View style={styles.timeDataMsgCon}>
                            <Text style={styles.timeDataMsgTxt}>{Language.pastTimeMsg}</Text>
                        </View>
                        <View style={styles.timeDateBtnCon}>
                            ...
                            <Button
                                onPress={() => setDateVisible(!dateVisible)}
                                // onPress={() => { setCheckSunday(1); setCheckSaturday(1); setDateVisible(false) }}
