...
import { FONT_FAMILY_BOLD, FONT_FAMILY_REGULAR, FONT_FAMILY_SEMIBOLD, PINK_COLOR_CODE, WHITE_COLOR_CODE, windowHeight, windowWidth } from '../../../../utils/constants';
            ...
            <ScrollView contentContainerStyle={{ flexGrow: 1 }}>
                            ...
                            renderItem={({ item, index }) => {
                                return (
                                    <TouchableOpacity onPress={() => props.previewsSerachFun(item, index)} style={{ margin: 2, padding: 2 }}>
                                        <Text style={{ color: PINK_COLOR_CODE, fontFamily: FONT_FAMILY_REGULAR }}>{item.cat_name} {'>'}</Text>
                                    </TouchableOpacity>
                                )
                            }}
                        />
                ...
                <View style={styles.productListCantainer}>
                    {props.productCategoryData ?
                        <>
                            {props.productCategoryData.categoryList ?
                                <View>
                                    <FlatList
                                        data={props.productCategoryData.categoryList}
                                        keyExtractor={(item, index) => index.toString()}
                                        // numColumns={2}
                                        horizontal
                                        ...
                                        renderItem={({ item, index }) => {
                                            return (
                                                <TouchableOpacity activeOpacity={0.6} onPress={() => props.SubCategoryListFun(item)} key={index} style={[styles.productItemCantainerCateg]} key={index}>
                                                    <View>
                                                        <View style={[styles.namePriceCotegory, { justifyContent: 'center', alignItems: 'center' }]}>
                                ...
                                : null}
                            {props.productCategoryData.productList ?
                                <FlatList
                                    data={props.productCategoryData.productList}
                                    keyExtractor={(item, index) => index.toString()}
                                    numColumns={2}
                                    enableEmptySections={true}
                                    refreshing={props.isFetching}
